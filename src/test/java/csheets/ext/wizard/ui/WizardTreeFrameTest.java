...
public class WizardTreeFrameTest {
	...
	public void setUp() throws FormulaCompilationException {
		...
		WizardFrame frame = new WizardFrame(null);
		frame.setVisible(false);
		CommonTree ast = new ExcelExpressionCompiler().
			compileTree("={POW(2;3)}");
	...
	/**
	 * Test of getTree method, of class WizardTreeFrame. Had to brute force it a
	 * bit.
	 */
	@Test
	public void ensureTreeIsProperlyConstructed() {
		...
		DefaultMutableTreeNode expResult = (DefaultMutableTreeNode) tree.
			getModel().getRoot();
		...
		DefaultMutableTreeNode result = (DefaultMutableTreeNode) instance.
		...
		if (verdict) {
			...
			if (verdict) {
				for (int i = 0; i < resultChild.getChildCount(); i++) {
					verdict = resultChild.getChildAt(i).toString().
						equals(expResultChild.getChildAt(i).toString());
					if (!verdict) {
						break;
					}
				}
