...
public class TcpServer extends Server {
	 ...
	 */
	protected void protocol(Socket socket, BufferedReader input,
							PrintWriter output) {
		...
		// Extra arguments.
		args.
			put("from", socket.getInetAddress().getHostAddress() + ":" + socket.
				getPort());
	...
	protected void protocol(String target, String message) {
		...
		try {
			Socket socket = new Socket(targetData[0], Integer.
									   parseInt(targetData[1]));
