...
public class SpreadsheetTable extends Grid implements SelectionListener {
    ...
    /**
     * Sets the data model of the table. Overridden to only accept instances of
     * the <code>SpreadsheetTableModel</code> class.
     *
     * @param dataModel the new data source for this table, must be a
     * <code>SpreadsheetTableModel</code>
     */
    ...
    public Cell getSelectedCell() {
        int activeColumn = getColumnModel().getSelectionModel().
     ...
     *
     * @return a two-dimensional array of the the currently selected cells in
    ...
    public Cell[][] getSelectedCells() {
        ...
        for (int row = 0; row < range.length; row++) {
            for (int column = 0; column < range[row].length; column++) {
                range[row][column] = spreadsheet.
                        getCell(columns[column], rows[row]);
            }
        }
        return range;
    }
    ...
    /**
     * Changes the current selection in the table. Overridden to update the user
     ...
     */
    /**
     * Sets the height for row to rowHeight, revalidates, and repaints. The
     * height of the cells in this row will be equal to the row height minus the
     * row margin.
     *
     ...
     * width of the cells in this column will be equal to the column width minus
     * the column margin.
     *
