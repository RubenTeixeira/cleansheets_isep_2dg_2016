...
public class ImportExportToDatabaseController {
    ...
    public void exportToDatabase(String driver, String connection, String username, String password, String tableName) throws SQLException, FormulaCompilationException {
    ...
    public void importFromDatabase(String driver, String connection, String username, String password, String tableName) throws SQLException, FormulaCompilationException {
        DatabaseConnector connector = DatabaseConnector.getInstance();
        connector.setConnection(driver, connection, username, password);
        Cell[][] cells = getSelectedCells();
    ...
    public void setCellContent(Cell[][] cells, ResultSet result) throws SQLException, FormulaCompilationException {
        ...
        for (int i = 0; i < cells[0].length; i++) {
            ...
            cells[0][i].setContent(rsmd.getColumnName(i));
        }
        int j = 1;
        while (result.next()) {
            for (int i = 1; i <= cells.length; i++) {
                ...
                cells[j][i].setContent(result.getString(i));
            }
            j++;
        }
    }
